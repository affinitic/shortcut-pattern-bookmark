...
chrome.runtime.onMessage.addListener(function(message, sender, callback) {
  if (message.type == GET_TAB_URL) {
    ...
    chrome.storage.local.get([message.key]).then((res) => {
      if (!res[message.key]) {
        callback([]);
        return true
      }
      ...
      console.log("🚀 ~ file: background.js:43 ~ chrome.storage.local.get ~ resultJson:", resultJson)
      callback(JSON.parse(resultJson));
      return true
    },
    (error) => {
      console.log("🚀 ~ file: chrome.js:45 ~ getStorageList ~ error:", error)
      callback([]);
      return true
    })
    return true;
  } else if (message.type == SET_LOCALSTORAGE) {
    console.log("write in local storage :",message.data);
    console.log("write in local storage :",JSON.stringify(message.data));
